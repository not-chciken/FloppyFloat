...
TEST(SoftFloatTests, F32ToU32) {
  ...
  for (auto rm : rounding_modes) {
    ...
    for (i32 i = 0; i < kNumIterations; ++i) {
      ...
      switch (rm) {
      case ::softfloat_round_near_even:
        ff_result = std::bit_cast<u32>(ff.F32ToU32<FloppyFloat::kRoundTiesToEven>(valuefa));
        break;
      case ::softfloat_round_minMag:
        ff_result = std::bit_cast<u32>(ff.F32ToU32<FloppyFloat::kRoundTowardZero>(valuefa));
        break;
      case ::softfloat_round_min:
        ff_result = std::bit_cast<u32>(ff.F32ToU32<FloppyFloat::kRoundTowardNegative>(valuefa));
        break;
      case ::softfloat_round_max:
        ff_result = std::bit_cast<u32>(ff.F32ToU32<FloppyFloat::kRoundTowardPositive>(valuefa));
        break;
      case ::softfloat_round_near_maxMag:
        ff_result = std::bit_cast<u32>(ff.F32ToU32<FloppyFloat::kRoundTiesToAway>(valuefa));
        break;
      default:
        break;
      }

      uint_fast32_t sf_result = ::f32_to_ui32(valuesfa, rm, true);

      std::cout << std::hex << sf_result << " " << ff_result << " " << valuesfa.v << std::endl;
      ASSERT_EQ(ff_result, sf_result);
      ASSERT_EQ(ff.invalid, static_cast<bool>(::softfloat_exceptionFlags & softfloat_flag_invalid));
      ASSERT_EQ(ff.division_by_zero, static_cast<bool>(::softfloat_exceptionFlags & softfloat_flag_infinite));
      ASSERT_EQ(ff.overflow, static_cast<bool>(::softfloat_exceptionFlags & softfloat_flag_overflow));
...
TEST(SoftFloatTests, F32ToF64) {
  ...
  for (auto rm : rounding_modes) {
    ...
    for (i32 i = 0; i < kNumIterations; ++i) {
      ...
      switch (rm) {
      case ::softfloat_round_near_even:
        ff_result = std::bit_cast<u64>(ff.F32ToF64(valuefa));
        break;
      case ::softfloat_round_minMag:
        ff_result = std::bit_cast<u64>(ff.F32ToF64(valuefa));
        break;
      ...
      u64 sf_result = ::f32_to_f64(valuesfa).v;
      ASSERT_EQ(ff_result, sf_result);
      ASSERT_EQ(ff.invalid, static_cast<bool>(::softfloat_exceptionFlags & softfloat_flag_invalid));
      ASSERT_EQ(ff.division_by_zero, static_cast<bool>(::softfloat_exceptionFlags & softfloat_flag_infinite));
      ASSERT_EQ(ff.overflow, static_cast<bool>(::softfloat_exceptionFlags & softfloat_flag_overflow));
